/* Keep Alive engine for ws server example

   This example code is in the Public Domain (or CC0 licensed, at your option.)

   Unless required by applicable law or agreed to in writing, this
   software is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR
   CONDITIONS OF ANY KIND, either express or implied.
*/

#include <esp_log.h>
#include <esp_system.h>
#include "freertos/FreeRTOS.h"
#include "freertos/queue.h"
#include "freertos/task.h"
#include "ws_helper.h"
#include "esp_timer.h"

typedef enum {
    NO_CLIENT = 0,
    CLIENT_FD_ADD,
    CLIENT_FD_REMOVE,
    CLIENT_UPDATE,
    CLIENT_ACTIVE,
    STOP_TASK,
} client_fd_action_type_t;

typedef struct {
    client_fd_action_type_t type;
    int fd;
    uint64_t last_seen;
} client_fd_action_t;

typedef struct ws_helper_storage {
    size_t max_clients;
    ws_check_client_alive_cb_t check_client_alive_cb;
    ws_check_client_alive_cb_t client_not_alive_cb;
    size_t keep_alive_period_ms;
    size_t not_alive_after_ms;
    void * user_ctx;
    QueueHandle_t q;
    client_fd_action_t clients[];
} ws_helper_storage_t;

typedef struct ws_helper_storage* ws_helper_t;

static const char *TAG = "ws_helper";

static uint64_t _tick_get_ms(void)
{
    return esp_timer_get_time()/1000;
}

// Goes over active clients to find out how long we could sleep before checking who's alive
static uint64_t get_max_delay(ws_helper_t h)
{
    int64_t check_after_ms = 30000; // max delay, no need to check anyone
    for (int i=0; i<h->max_clients; ++i) {
        if (h->clients[i].type == CLIENT_ACTIVE) {
            uint64_t check_this_client_at = h->clients[i].last_seen + h->keep_alive_period_ms;
            if (check_this_client_at < check_after_ms + _tick_get_ms()) {
                check_after_ms = check_this_client_at - _tick_get_ms();
                if (check_after_ms < 0) {
                    check_after_ms = 1000; // min delay, some client(s) not responding already
                }
            }
        }
    }
    return check_after_ms;
}


static bool update_client(ws_helper_t h, int sockfd, uint64_t timestamp)
{
    for (int i=0; i<h->max_clients; ++i) {
        if (h->clients[i].type == CLIENT_ACTIVE && h->clients[i].fd == sockfd) {
            h->clients[i].last_seen = timestamp;
            return true;
        }
    }
    return false;
}

static bool remove_client(ws_helper_t h, int sockfd)
{
    for (int i=0; i<h->max_clients; ++i) {
        if (h->clients[i].type == CLIENT_ACTIVE && h->clients[i].fd == sockfd) {
            h->clients[i].type = NO_CLIENT;
            h->clients[i].fd = -1;
            return true;
        }
    }
    return false;
}
static bool add_new_client(ws_helper_t h,int sockfd)
{
    for (int i=0; i<h->max_clients; ++i) {
        if (h->clients[i].type == NO_CLIENT) {
            h->clients[i].type = CLIENT_ACTIVE;
            h->clients[i].fd = sockfd;
            h->clients[i].last_seen = _tick_get_ms();
            return true; // success
        }
    }
    return false;
}

static void keep_alive_task(void* arg)
{
    ws_helper_storage_t *_helper_storage = arg;
    bool run_task = true;
    client_fd_action_t client_action;
    
    while (run_task) {
        if (xQueueReceive(_helper_storage->q, (void *) &client_action,
                get_max_delay(_helper_storage) / portTICK_PERIOD_MS) == pdTRUE) {
            switch (client_action.type) {
                case CLIENT_FD_ADD:
                    if (!add_new_client(_helper_storage, client_action.fd)) {
                        ESP_LOGE(TAG, "Cannot add new client");
                    }
                    break;
                case CLIENT_FD_REMOVE:
                    if (!remove_client(_helper_storage, client_action.fd)) {
                        ESP_LOGE(TAG, "Cannot remove client fd:%d", client_action.fd);
                    }
                    break;
                case CLIENT_UPDATE:
                    if (!update_client(_helper_storage, client_action.fd, client_action.last_seen)) {
                        ESP_LOGE(TAG, "Cannot find client fd:%d", client_action.fd);
                    }
                    break;
                case STOP_TASK:
                    run_task = false;
                    break;
                default:
                    ESP_LOGE(TAG, "Unexpected client action");
                    break;
            }
        } else {
                // timeout: check if PING message needed
                for (int i=0; i<_helper_storage->max_clients; ++i) {
                    if (_helper_storage->clients[i].type == CLIENT_ACTIVE) {
                        if (_helper_storage->clients[i].last_seen + _helper_storage->keep_alive_period_ms <= _tick_get_ms()) {
                            ESP_LOGD(TAG, "Haven't seen the client (fd=%d) for a while", _helper_storage->clients[i].fd);
                            if (_helper_storage->clients[i].last_seen + _helper_storage->not_alive_after_ms <= _tick_get_ms()) {
                                ESP_LOGE(TAG, "Client (fd=%d) not alive!",  _helper_storage->clients[i].fd);
                                _helper_storage->client_not_alive_cb(_helper_storage, _helper_storage->clients[i].fd);
                            } else {
                                _helper_storage->check_client_alive_cb(_helper_storage, _helper_storage->clients[i].fd);
                            }
                        }
                    }
                }
            }
    }
    vQueueDelete(_helper_storage->q);
    free(_helper_storage);

    vTaskDelete(NULL);
}

ws_helper_t ws_helper_start(ws_helper_config_t *config)
{
    size_t queue_size = config->max_clients/2;
    size_t client_list_size = config->max_clients + queue_size;
    ws_helper_t _helper_storage = calloc(1,
            sizeof(ws_helper_storage_t) + client_list_size* sizeof(client_fd_action_t));
    if (_helper_storage == NULL) {
        return false;
    }
    _helper_storage->check_client_alive_cb = config->check_client_alive_cb;
    _helper_storage->client_not_alive_cb = config->client_not_alive_cb;
    _helper_storage->max_clients = config->max_clients;
    _helper_storage->not_alive_after_ms = config->not_alive_after_ms;
    _helper_storage->keep_alive_period_ms = config->keep_alive_period_ms;
    _helper_storage->user_ctx = config->user_ctx;
    _helper_storage->q =  xQueueCreate(queue_size, sizeof(client_fd_action_t));
    if (xTaskCreate(keep_alive_task, "keep_alive_task", config->task_stack_size,
                    _helper_storage, config->task_prio, NULL) != pdTRUE) {
        ws_helper_stop(_helper_storage);
        return false;
    }
    return _helper_storage;
}

void ws_helper_stop(ws_helper_t h)
{
    client_fd_action_t stop = { .type = STOP_TASK };
    xQueueSendToBack(h->q, &stop, 0);
    // internal structs will be de-allocated in the task
}

esp_err_t ws_helper_add_client(ws_helper_t h, int fd)
{
    client_fd_action_t client_fd_action = { .fd = fd, .type = CLIENT_FD_ADD};
    if (xQueueSendToBack(h->q, &client_fd_action, 0) == pdTRUE) {
        return ESP_OK;
    }
    return ESP_FAIL;
}

esp_err_t ws_helper_remove_client(ws_helper_t h, int fd)
{
    bool _is_ws = false;
    for (int i=0; i<h->max_clients; ++i) {
        if (h->clients[i].fd == fd) _is_ws = true;
    }  
    if (_is_ws) {
        client_fd_action_t client_fd_action = { .fd = fd, .type = CLIENT_FD_REMOVE};
        if (xQueueSendToBack(h->q, &client_fd_action, 0) == pdTRUE) {
            return ESP_OK;
        }
    }
    return ESP_FAIL;
}

esp_err_t ws_helper_client_is_active(ws_helper_t h, int fd)
{
    client_fd_action_t client_fd_action = { .fd = fd, .type = CLIENT_UPDATE,
                                            .last_seen = _tick_get_ms()};
    if (xQueueSendToBack(h->q, &client_fd_action, 0) == pdTRUE) {
        return ESP_OK;
    }
    return ESP_FAIL;

}

void ws_helper_set_user_ctx(ws_helper_t h, void *ctx)
{
    h->user_ctx = ctx;
}

void* ws_helper_get_user_ctx(ws_helper_t h)
{
    return h->user_ctx;
}
