...
void espnow_mem_print_record(void)
{
    ...
    for (int i = 0; i < MEM_DBG_INFO_MAX; i++) {
        if (g_mem_info[i].size) {
            ESP_LOGI(TAG, "(%ld) <%s: %d> ptr: %p, size: %u", g_mem_info[i].timestamp, g_mem_info[i].tag, g_mem_info[i].line,
                     g_mem_info[i].ptr, g_mem_info[i].size);
            total_size += g_mem_info[i].size;
        }
    }

    ESP_LOGI(TAG, "Memory record, num: %ld, size: %ud", g_mem_count, total_size);
}
