...
ICONS_PATH = ASSETS_PATH / "mdi"
DEFAULT_CONFIG = SCRIPT_DIR / "configuration.yaml"
DEFAULT_FONT: str = "GlacialIndifference-Bold.otf"
DEFAULT_MDI_ICONS = {
    "light": "lightbulb",
...
class Button(BaseModel, extra="forbid"):  # type: ignore[call-arg]
    ...
    service_hold: str | None = Field(
        default=None,
        allow_template=True,
        description="The `service` that will be called when the button is pressed.",
    )
    service_data_hold: dict[str, Any] | None = Field(
        default=None,
        allow_template=True,
        description="The `service_data` that will be passed to the `service` when the button is pressed."
        " If empty, the `entity_id` will be passed.",
    )
    target_hold: dict[str, Any] | None = Field(
        default=None,
        allow_template=True,
        description="The `target` that will be passed to the `service` when the button is pressed.",
    )    
    ...
    gradient_ratio: float = Field(
        default=0,
        allow_template=False,
        description="When specifying `icon` and `entity_id`, if the state is `off`, the icon will be converted to grayscale.",
    )  
    gradient_filename: str | None = Field(
        default=None,
        allow_template=True,
        description="The icon filename to display on the button."
    )             
    ...
    special_type_data_hold: Any | None = Field(
        default=None,
        allow_template=True,
        description="Data for the special type of button."
        " If `go-to-page`, the data should be an `int` or `str` (name of the page)."
    ...
    def render_icon(  # noqa: PLR0912 PLR0915
        ...
        *,
        ...
        gradient_filename = button.gradient_filename
        text_color = button.text_color or "white"
        label_color = button.label_color or "white"
        ...
        if image is None:
            image = _init_icon(
                ...
                icon_mdi=icon_mdi,
                icon_mdi_margin=icon_mdi_margin,
                icon_mdi_color=_named_to_hex(button.icon_mdi_color or text_color, int(opacity)),
                size=size,
                icon_offset_x=self.icon_offset_x,
                icon_offset_y=self.icon_offset_y,
                icon_size=self.icon_size,
                gradient=self.gradient,  
                gradient_ratio=self.gradient_ratio, 
        ...
        _add_text(
            ...
            text=text,
            text_color=text_color if not key_pressed else "white",
            text_offset_x=self.text_offset_x,
            text_offset_y=self.text_offset_y,           
            label_size=self.label_size,
            label=label,
            label_color=label_color if not key_pressed else "white",
    ...
    def _validate_special_type(  # noqa: PLR0912
        ...
        if special_type == "brightness" and not isinstance(v, (int, str)):
            msg = (
                "If special_type is brightness, special_type_data must be an int or str"
            )
            raise AssertionError(msg)        
        if (
...
class Config(BaseModel):
    ...
    def next_page(self) -> Page:
        ...
        while self.pages[self._current_page_index].hidden == True:
            self._current_page_index = self.next_page_index
...
def _update_state(
    ...
    if data["type"] == "event":
        ...
        if event_data["event_type"] == "state_changed":
            ...
            if eid in config.state_entity_id:
            ...
            if eid in config.brightness_entity_id:
                is_on = complete_state[config.state_entity_id]["state"] == "on"
                Number.global_brightness = int(float(complete_state[config.brightness_entity_id]["state"]))
                if is_on and time.time() - Number.last_set_brightness > 3:
                    # console.log(complete_state[config.brightness_entity_id]["state"])
                    deck.set_brightness(int(float(complete_state[config.brightness_entity_id]["state"])))
                return
...
def _states(
    ...
    *,
    ...
    state = str(_maybe_number(state, rounded=rounded))
    if with_unit:
...
def _rgb_to_hex_alpha(rgb: tuple[int, int, int, int]) -> str:
    """Convert an RGB color to a hex color."""
...
def _named_to_hex(color: str, opacity: int) -> str:
    """Convert a named color to a hex color."""
    if color.startswith("["):
        list = color[1:-1].split(',')
        if opacity != 0:
            list.append(opacity) 
            return _rgb_to_hex_alpha(tuple([int(i) for i in list]))
        return _rgb_to_hex(tuple([int(i) for i in list]))
    if color == "": color = (30,30,30)	
...
def _download_and_save_mdi(icon_mdi: str) -> Path:
    ...
    if ":" in icon_mdi:
        icon_mdi = re.search(r'(?<=:).*', icon_mdi).group()
    filename_svg = ICONS_PATH / f"{icon_mdi}.svg"
...
def _init_icon(
    *,
    ...
    """Initialize the icon."""
    if icon_filename is not None and ":" not in icon_filename:
    ...
    elif icon_filename is not None and ":" in icon_filename:
        which, id_ = icon_filename.split(":", 1)
        if which == "spotify":
            filename = _to_filename(icon_filename, ".jpeg")
            # copy to avoid modifying the cached image
            image = _download_spotify_image(id_, filename).copy()
        if which == "url":
            print(id_)
            ...
            print(filename)
            # copy to avoid modifying the cached image
            image = _download_image(id_, filename, size).copy().convert('RGBA')
            if image.size != size:
        ...
        mask = Image.open(ASSETS_PATH / Path("mask.png"))       
        background = Image.new("RGBA", size, icon_mdi_color)
        # background.paste(mask,(0,0))
        if mask.size != size:
            mask = mask.resize(size)       
        icon = Image.alpha_composite(background, mask)
    elif gradient == 2: 
        mask = Image.open(ASSETS_PATH / Path("mask2.png"))       
        background = Image.new("RGBA", size, icon_mdi_color)
        # background.paste(mask,(0,0))
        if mask.size != size:
            mask = mask.resize(size)       
        ...
        mask_path = Path(gradient_filename)
        path = mask_path if mask_path.is_absolute() else ASSETS_PATH / mask_path
    ...
    if (icon_filename is not None and ":" not in icon_filename) or gradient:# copy to avoid modifying the cached image
        return Image.alpha_composite(icon, icon2)
    elif (icon_filename is not None and ":" in icon_filename):
        return Image.alpha_composite(image, icon2)
    else:
        return icon2
...
def _add_text(
    *,
    ...
    if text_color.startswith("["):
        list = text_color[1:-1].split(',')
        text_color = tuple([int(i) for i in list])
    if label_color.startswith("["):
        list = label_color[1:-1].split(',')
        label_color = tuple([int(i) for i in list])        
    draw.text(
        (text_offset_x, image.height / 2 + text_offset_y),
        text=text,
        font=font,
        anchor="ls",
        fill=text_color,
    ...
    draw.text(
        (label_offset_x, image.height / 2 + label_offset_y),
        text=label,
        font=ImageFont.truetype(str(ASSETS_PATH / font_filename), label_size),
        anchor="ls",
        fill=label_color,
        align="left",
    )    
...
def turn_on(config: Config, deck: StreamDeck, complete_state: StateDict) -> None:
    ...
    config._is_on = True
    update_all_key_images(deck, config, complete_state)
    deck.set_brightness(Number.global_brightness or config.brightness)
...
async def _handle_key_press_hold(
    websocket: websockets.WebSocketClientProtocol,
    complete_state: StateDict,
    config: Config,
    button: Button,
    ...
    if button.special_type_hold == "next-page":
        config.next_page()
        update_all()
    elif button.special_type_hold == "previous-page":
        config.previous_page()
        update_all()
    elif button.special_type_hold == "go-to-page":
        assert isinstance(button.special_type_data_hold, (str, int))
        config.to_page(button.special_type_data_hold)  # type: ignore[arg-type]
        update_all()
        return  # to skip the _detached_page reset below
    elif button.special_type_hold == "turn-off":
        turn_off(config, deck)
        await call_service(websocket, "input_boolean.turn_off", {"entity_id": config.state_entity_id})
    elif button.special_type_hold == "light-control":
        assert isinstance(button.special_type_data_hold, dict)
        page = _light_page(
            entity_id=button.entity_id,
            n_colors=10,
            colormap=button.special_type_data_hold.get("colormap", None),
            colors=button.special_type_data_hold.get("colors", None),
            color_temp_kelvin=button.special_type_data_hold.get("color_temp_kelvin", None),
        )
        config._detached_page = page
        update_all()
        return  # to skip the _detached_page reset below
    elif button.special_type_hold == "reload":
        config.reload()
        update_all()
        return
    elif button.service_hold is not None:
        button = button.rendered_template_button(complete_state)
        if button.service_data_hold is None:
            service_data = {}
        else:
            service_data = button.service_data_hold
        console.log(f"Calling service {button.service_hold} with data {service_data}")
        assert button.service_hold is not None  # for mypy
        await call_service(websocket, button.service_hold, service_data, button.target_hold)
...
async def _handle_key_press(
    ...
    if button.special_type == "next-page":
        ...
        config.previous_page()
        update_all()
    elif button.special_type == "brightness":
        assert isinstance(button.special_type_data, (str, int))
        Number.global_brightness = button.special_type_data
        deck.set_brightness(button.special_type_data)
        Number.last_set_brightness = time.time()
        await call_service(websocket, "input_number.set_value", {"entity_id": config.brightness_entity_id, "value": Number.global_brightness})
        return  # to skip the _detached_page reset below        
    elif button.special_type == "go-to-page":
        assert isinstance(button.special_type_data, (str, int))
...
def _on_press_callback(
    ...
    complete_state: StateDict,
    config: Config,
) -> Callable[[StreamDeck, int, bool], Coroutine[StreamDeck, int, bool]]:
    async def key_change_callback(
        ...
        if key_pressed:
            Number.last_pressed = time.time()
        else:
            diff =   time.time() - Number.last_pressed
        ...
        try:
            ...
            # if key_pressed:
            #     await _handle_key_press(websocket, complete_state, config, button, deck)          
            if not key_pressed and diff < 0.4:
                await _handle_key_press(websocket, complete_state, config, button, deck)
            elif not key_pressed:
                await _handle_key_press_hold(websocket, complete_state, config, button, deck)
...
def _url_to_filename(url: str, hash_len: int = 8) -> Path:
    ...
    h = hashlib.sha256(f"{domain}".encode()).hexdigest()[:hash_len]
    # extension = Path(path).suffix
    filename = f"{h}.png"
    return CACHE_PATH / Path(filename)
...
def _convert_svg_to_png(
    *,
    ...
    size: tuple[int, int] = (ICON_PIXELS, ICON_PIXELS),
    icon_offset_x, icon_offset_y, icon_size
) -> Image.Image:
    ...
    try:
        ...
        svg_tree.attrib["width"] = icon_size
        svg_tree.attrib["height"] = icon_size
...
async def run(
    ...
    async with setup_ws(host, token, protocol) as websocket:
        ...
        Number.global_brightness = config.brightness
        Number.last_set_brightness = time.time()
        await call_service(websocket, "input_boolean.turn_on", {"entity_id": config.state_entity_id})
        await call_service(websocket, "input_number.set_value", {"entity_id": config.brightness_entity_id, "value": Number.global_brightness})
