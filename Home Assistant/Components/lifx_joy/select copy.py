...
async def async_setup_entry(
    ...
    if lifx_features(coordinator.device)["matrix"] is True:
        entities.append(JoyThemeSelect(coordinator))
...
class JoyThemeSelect(LIFXEntity, SelectEntity):
    ...
    def __init__(self, coordinator: LIFXUpdateCoordinator) -> None:
        ...
        self._attr_unique_id = f"{coordinator.serial_number}_theme"
        self._attr_current_option = None
        try:
            self._attr_options = list(self.coordinator.theme_list.keys())
        except:
            self._attr_options = ["default10", "default20"]

    @property
    def current_option(self) -> str | None:
        """Return the current selected preset."""
        return str(self.coordinator.current_theme)
    ...
    def _async_update_attrs(self) -> None:
        """Update attrs from coordinator data."""
        try:
            # _LOGGER.debug(f"Async setting {self.coordinator.theme_list}")
            # _LOGGER.debug(f"Async setting {list(self.coordinator.theme_list.keys())}")
            self._attr_current_option = str(self.coordinator.current_theme)
            self._attr_state    = str(self.coordinator.current_theme)
            self._attr_options  = list(self.coordinator.theme_list.keys())
        except:
            self._attr_options = ["None"]
        _LOGGER.debug(f"self._attr_options {self._attr_options}, self._attr_current_option {self._attr_current_option}, self.coordinator.current_theme {self.coordinator.current_theme}")
        _LOGGER.debug(f"\n{self.options}\n{self.current_option}\n{self.state}")
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        self.coordinator.current_theme = option
        super()._handle_coordinator_update()
        
        if self.coordinator.theme_list == None:
            _LOGGER.debug(f">>>>>\n self.coordinator.theme_list {self.coordinator.theme_list}")
            return
        else:
            theme_list = self.coordinator.theme_list
    
        if option in theme_list:
            palette = theme_list[option]
        else:
            palette = [[50,100,100,1500]]
        
        _LOGGER.debug(f">>>>>\n self.coordinator.theme {self.coordinator.theme}")
        
        if isinstance(palette, list) and palette != []:
            await self.coordinator.async_set_tile64_select_theme(palette)
