...
# Failed to call service light/turn_on. value must be at most 360 @ data['palette'][0]

    async def set64_effect(self, **kwargs: Any) -> None:
        """Start an effect with default parameters."""

        array = self.manager.palette

        # array = np.array(self.coordinator.palette)
        # array[:,0] = (array[:,0] / 182.04).astype(int).clip(0, 360) 
        # array[:,1:3] = (array[:,1:3] / 655.35).astype(int).clip(0, 100)
        # array = array.tolist()
    ...
    async def set_state(self, **kwargs: Any) -> None:
        ...
        _LOGGER.debug(f">>>>> Set State {kwargs}, Current effect {self.effect}, is_on {self.is_on}")
        ...
        if ATTR_EFFECT in kwargs:
            _LOGGER.debug(f">>>>> ATTR_EFFECT {kwargs[ATTR_EFFECT]}")
        ...
        _LOGGER.debug(f">>>>> hsbk {hsbk} | {power_on}, {power_off}")
